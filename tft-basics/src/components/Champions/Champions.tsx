...
const Champions = () => {
    const champions: string[] = getSet11ChampionsName();

    const [filteredChampions, setFilteredChampions] = useState<string[]>(champions);
    ...
    useEffect(() => {
        if (filter === "") {
            ...
            setFilteredChampions((fc) => fc.filter((champ) => champ.toLocaleLowerCase().includes(filter)))
        }
    },[filter])
    ...
    return (
        <>
            ...
            <div className="flex flex-row justify-center gap-4 m-2">
                <button className="p-1 bg-indigo-800 rounded-sm">Cost</button>
            ...
            <div className="flex justify-center">
                ...
                {filteredChampions && filteredChampions.map(champ => (
                    <Link to={`/champions/${champ}`} key={champ} className="grid place-items-center cursor-pointer hover:opacity-70 h-18 w-18">
                        <img className="h-14 w-14 rounded-full" src={`/src/assets/champions/${champ}.png`} alt={champ} />
                        <p>{champ}</p>
                    </Link>
                ))}
